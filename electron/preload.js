...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  auth: {
    ...
    loginPin:          (userId, pin)               => ipcRenderer.invoke('auth:loginPin',          { userId, pin }),
    loginPassword:     (username, password)        => ipcRenderer.invoke('auth:loginPassword',     { username, password }),
    verificarAdminPin: (pin)                       => ipcRenderer.invoke('auth:verificarAdminPin', { pin }),
    logout:            (userId, username)          => ipcRenderer.invoke('auth:logout',            { userId, username }),
    cerrarApp:         (userId, username)          => ipcRenderer.invoke('auth:cerrarApp',         { userId, username }),
    crearUsuario:      (solicitanteId, data)       => ipcRenderer.invoke('auth:crearUsuario',      { solicitanteId, ...data }),
    logs:              (userId, limit)             => ipcRenderer.invoke('auth:logs',              { userId, limit }),
    registrarAccion:   (userId, username, action, details) =>
                          ipcRenderer.invoke('auth:registrarAccion', { userId, username, action, details }),
  },
